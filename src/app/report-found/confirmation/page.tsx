...
export default function ReportFoundConfirmationPage() {
    ...
    <div className="bg-background text-on-surface antialiased min-h-screen flex flex-col relative overflow-hidden font-body-md">
      ...
      <main className="grow z-10 px-container-margin pt-12 pb-40 flex flex-col items-center justify-center gap-lg max-w-md mx-auto w-full text-center">
        <header className="flex flex-col items-center gap-2">
          ...
          <h1 className="font-headline-md text-headline-md text-on-surface">Merci d&apos;aider à le rendre ! 🎉</h1>
          <p className="font-body-lg text-body-lg text-on-surface-variant">Votre découverte a bien été enregistrée.</p>
        </header>
        ...
        <div className="w-full bg-surface-container-lowest rounded-2xl soft-shadow overflow-hidden">
          ...
          <div className="p-5 flex flex-col gap-4 text-left">
            <div className="flex justify-between items-start">
              <div>
                <h2 className="font-headline-sm text-headline-sm text-on-surface">{draft.objectName || draft.categoryLabel || "Objet trouvé"}</h2>
                <div className="flex items-center gap-1 text-on-surface-variant mt-1">
                  <span className="material-symbols-outlined text-[16px]">calendar_today</span>
                  <span className="font-body-md text-[13px]">Trouvé le {draft.date ? new Date(draft.date).toLocaleDateString("fr-FR") : "2 septembre 2026"}</span>
                </div>
              </div>
            ...
            <p className="font-body-md text-[13px] text-on-surface-variant pt-2 border-t border-surface-variant/50">
              Objely préviendra son propriétaire dès qu&apos;une correspondance sera confirmée avec votre découverte.
            </p>
          </div>
        </div>
      </main>
