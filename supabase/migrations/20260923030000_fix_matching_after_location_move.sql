-- Regression from 20260923010000_protect_item_location.sql: dropping
-- items.location broke the two server-side matching functions that read it
-- directly (find_best_match_candidate, explain_match_criteria) — every
-- location-aware match score/criteria computation started failing with
-- "column li.location does not exist". Found via a second round of
-- penetration testing (an insert that should have been rejected by RLS
-- instead surfaced this SQL error first). Both are SECURITY-context-free
-- `language sql` functions with no RLS concerns of their own — they just
-- need to read the exact location from its new home, item_locations,
-- instead of the dropped column.
